...
function assemble(Kg, Ke, connectivity; dofs_per_node=2)
    # --- 1. DOF map ---
    num_local_nodes = length(connectivity)
    num_local_dofs = num_local_nodes * dofs_per_node
    dof_map = Vector{Int}(undef, num_local_dofs)

    for i in 1:num_local_nodes
        global_node = connectivity[i]
        global_dof_start = (global_node - 1) * dofs_per_node
        local_dof_start = (i - 1) * dofs_per_node
        for j in 1:dofs_per_node
            dof_map[local_dof_start + j] = global_dof_start + j
        end
    end

    # --- 2. Assembly loops ---
    for i in 1:num_local_dofs  # Local row index
        I = dof_map[i]       # Global row index
        for j in 1:num_local_dofs # Local col index
