    ...
    try {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (s.ShapeType() == TopAbs_WIRE) wires[i].wire = TopoDS::Wire(s);
                ...
                else return nullptr; // Input was not a wire
            }

            // Check if wire has ANY edges
            TopExp_Explorer eExp(wires[i].wire, TopAbs_EDGE);
            if (!eExp.More()) return nullptr; // Empty wire
        ...
        if (cloudSize > 500) cloudSize = 500;
        ...
        for (size_t i = 0; i < n - 1; ++i) {
            ...
            while (exp.More()) {
                ...
                if (!exp.More() && !w1.isClosed) {
                    ...
                    for (size_t k = 0; k < w1.cloud.size(); ++k) {
                        ...
                        if (d < minDLast) { minDLast = d; uLast = w1.params[k]; }
                    }
            ...
            for (int k = 0; k < numSegments; ++k) {
                ...
                if (w1.isClosed) {
                    if (u1_end < u1_start) u1_end += w1.period;
                ...
                if (w2.isClosed) {
                    if (u2_end < u2_start) u2_end += w2.period;
