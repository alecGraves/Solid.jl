...
/* ------------------------------------------------------------------
   All wrappers are wrapped in an extern "C" block so that the
   generated symbols are unmangled and can be linked from C code.
   --------------------------------------------------------------- */
extern "C" {
...
Shape mucad_sweep(Shape profile,
                    double x0, double y0, double z0,
                    double x1, double y1, double z1)
{
    return mucadcpp_sweep(profile, x0, y0, z0, x1, y1, z1);
}
...
{
    return mucadcpp_write_iges(s, filename);
}

int mucad_write_obj(Shape s, const char* filename)
{
    return mucadcpp_write_obj(s, filename);
}
